...
DATA.META = {
  ...
  g_star_time:   { st:'b_star', fac:'life', name:'星読みの加護', desc:'強い色違いの敵の出現を3%緩和(最大45%)', max:15, cost:gcost(1650,1.55) },
  g_star_chart:  { st:'b_star', fac:'lore', name:'星図の導き', desc:'移動速度 +1.5%', max:15, cost:gcost(1400,1.5), effMul:{speed:.015} },
  g_star_sands:  { st:'b_star', fac:'lore', name:'刻の砂時計', desc:'「時の砂」の効果量 +4%', max:15, cost:gcost(1500,1.5) },
  ...
  g_green_charisma:{ st:'b_green', fac:'lore', name:'森の歌声', desc:'「カリスマの歌」の効果量 +4%', max:15, cost:gcost(650,1.5) },
  ...
  g_white_magnet:{ st:'b_white', fac:'lore', name:'白亜の磁鉄', desc:'「マグネットフィールド」の効果量 +4%', max:15, cost:gcost(2350,1.5) },
  ...
  g_black_vamp:  { st:'b_black', fac:'war',  name:'血の契約',   desc:'「吸血の刻印」の効果量 +4%', max:15, cost:gcost(2250,1.5) },
  ...
  g_ember_burn:  { st:'b_ember', fac:'war',  name:'燃えさしの祝福', desc:'全攻撃に4%で炎上を付与', max:10, cost:gcost(1500,1.5) },
  ...
  g_forge_banner:{ st:'b_forge', fac:'war',  name:'軍旗の鍛造', desc:'「ウォーバナー」の効果量 +4%', max:15, cost:gcost(6100,1.45) },
  ...
  g_storm_boots: { st:'b_storm', fac:'lore', name:'追い風',     desc:'「ヘルメスの靴」の効果量 +3%', max:10, cost:gcost(8450,1.5) },
  ...
  g_grave_curse: { st:'b_grave', fac:'war',  name:'墓標の呪詛', desc:'「衰弱の呪印」の効果量 +4%', max:15, cost:gcost(3600,1.5) },
  ...
  // --- 最果て ---
  g_end_beyond:  { st:'b_end',   fac:'war',  name:'終焉超越',   desc:'色違いの敵の出現をさらに2%緩和 / 全ダメージ+8%', max:20, cost:gcost(30500,1.55) },
...
// 実物として置く。props=飾り(種類kindごとの描画はhub.js) / obst=通れない地形の障害(円)。
// 座標はマップ中央基準。施設や住民の立ち位置(x:±140〜±520, y:-330〜260)は避けて置く。
DATA.HUB_THEME = {
  ...
  main: { floor:'#131a2b',
    ...
    obst:[ { x:-450, y:130, r:108 }, { x:-150, y:-284, r:16 }, { x:150, y:-284, r:16 },
           { x:-268, y:-252, r:12 }, { x:268, y:-252, r:12 },
           { x:-92, y:-192, r:14 }, { x:92, y:-192, r:14 } ] },
  ...
  b_east: { floor:'#1b1f2e',
    ...
    obst:[ { x:-540, y:-230, r:34 }, { x:540, y:-230, r:34 } ] },
  ...
  b_spa: { floor:'#20242a',
    ...
    obst:[ { x:-460, y:60, r:125 }, { x:480, y:-160, r:95 } ] },
  ...
  b_black: { floor:'#171b1f',
    ...
    obst:[ { x:-540, y:-130, r:36 }, { x:540, y:70, r:32 } ] },
  ...
  b_moon: { floor:'#141a33',
    ...
    obst:[ { x:-520, y:-205, r:32 }, { x:520, y:-205, r:32 } ] },
  ...
  b_frost: { floor:'#1c2833',
    ...
    obst:[ { x:-540, y:-200, r:44 }, { x:540, y:-175, r:38 }, { x:480, y:120, r:100 } ] },
  ...
  b_sun: { floor:'#2b2010',
    ...
    obst:[ { x:-540, y:-230, r:40 }, { x:540, y:-230, r:40 } ] },
  b_void: { floor:'#120f1c', voidmist:1,
    ...
    obst:[ { x:-520, y:-200, r:38 }, { x:520, y:-185, r:34 } ] },
  b_sea: { floor:'#123240',
...
DATA.SIDEQUESTS = {
  ...
  b_end: [
    { id:'sq_ou', npc:'npc_sage', npcName:'城主オウ', type:'hunt', enemy:'hornedimp', count:10,
      ...
      done:['見事だ。…では約束通り、余の話をしよう。','余の名はオウ。かつて――お前と同じ「死に戻り」であった。','幾百の生を重ね、広場に祭壇を築き、写本を残し、この果てまで辿り着いた。','その懐の切れ端。北の砦の兵に託したのは余だ。裏の字も、余の手よ。……よく、ここまで持ってきた。','砦の老兵が見た旅人も、遺跡の装置を調整した者も、鍛冶神に得物を打たせた者も…全て余だ。','還る魂と、還らぬ魂。その違いは、余にも分からぬ。ただ還る者は決まって「門を通らずに」戻る。虚無の隠者も、同じことを言うたであろう。','そして三十分。あれは世界の寿命ではない。……誰かが、そう決めておる。余が辿り着けたのは、そこまでじゃ。決めた者の顔は、ついぞ見ておらぬ。','だが終焉の刻の、その先へは…行けなんだ。だから余はここで、次の魂を待っておった。','余が越えられなかった終焉の刻を、超えてみせよ。その先で、また会おう。'],
...
DATA.QUESTS = {
  ...
  b_north: { npc:'npc_elder', npcName:'老兵ガルド', after:['おお、戻ったか。砦の火は今日も絶やしとらんぞ。'], type:'hunt', nearLoc:true, enemy:'skeleton', count:4,
    ...
    done:['見事じゃ…!これでゲートに火を灯せる。','…お前さん、「死に戻り」の魂じゃな。死んでも魂があの広場に還って、そこで鍛えた力ごと戻ってくる――そういう魂よ。ワシら年寄りには分かる。','昔もう一人だけ、同じ魂を見た。魂の広場の祭壇を築いた御仁じゃ。','その御仁は「果ての城を目指す」と言い残して、消えた。…お前さんは二人目じゃよ。','見ろ、ゲートが灯った。お前さんの魂の広場と、この砦が繋がったんじゃ。いつでも戻ってこい。','…だがな、解せんことがある。先の御仁もお前さんも、死んで還る。ワシの部下たちは、誰ひとり還らなんだ。……何が違うんじゃろうな。'] },
  b_east: { npc:'npc_scholar', npcName:'考古学者リナ', after:['あ、おかえり!いま新しい碑文を写してたの。研究、ちゃんと進んでるわよ。'], type:'hunt', nearLoc:true, enemy:'goblin', count:4,
    intro:['きゃっ!?…な、なんだ人間か。私はこの遺跡を調べてる研究者。','この遺跡の正体はね、大昔の転送ゲート。魂の広場に繋がる装置なの。','なのにゴブリンたちがいじり回して…壊れちゃう前に4体追い払って!'],
    done:['助かった〜!…ほら、ゲートが目を覚ました。','これで魂の広場から直接ここへ来られるわよ。研究の成果、期待してて。','…ねえ、ひとつ気になってることがあるの。夜、東の海が金色に光ることがあるのよ。私の観測記録にも三度。原因は不明。','遺跡で手一杯で調べに行けないけど…あれ、絶対に何かあるわ。'] },
  ...
  b_bones: { npc:'npc_smith', npcName:'骨商人ザリ', after:['へっへっへ、また来たな。今日も骨の相場は上々よ。'], type:'delivery', need:{ coins:500, mats:{bone:18, hide:8} },
    ...
    done:['まいどあり!ほれ、見事な骨組みだろう。ゲートも灯った。','ここはアンタの取引所だ。','へっへ…骨の相場でな、近ごろ妙なんだ。西の果てから流れてくる骨が、どれも同じ日に死んどる。同じ日にだぞ?'] },
  b_ember: { npc:'npc_smith', npcName:'火の子エン', after:['あっ、おかえり!御神火、今日も元気だよ!'], type:'hunt', nearLoc:true, enemy:'orc', count:4,
  ...
  b_void: { npc:'npc_sage', npcName:'虚無の囁き', after:['……来たか。静けさは、保たれておる。'], type:'hunt', nearLoc:true, enemy:'demon', count:5,
    ...
    done:['……良い。静けさが戻り、ゲートが開いた。','虚無はお前を通す。','……ひとつ教えておこう。全ての魂は門を通る。お前は通らずに戻ってきた。それが何を意味するかは、まだ言えぬ。'] },
...
const PORT_SMITH = {
  ...
  p_e:  { open:'おう、あの船かい?嵐で浜に打ち上がってたのを、俺が引き取ったんだ。', req:'直してほしけりゃ材料と手間賃だ。話はそれからだぜ。',
          fix:'…よし、直った!マストも帆も新品同様だ。', close:'初めての航海にゃもってこいの船に仕上げたぜ。良い風を!',
          mate:'そうだ、海岸を北へ辿りゃ北東の、南へ下りゃ南東の浜にも船乗り仲間がいる。夜通し回ってる灯台の光が目印だ。' },
  p_ne: { open:'ん?あの船が気になるか。持ち主は嵐の晩に消えちまってな…俺が預かってる。', req:'直すなら材料と手間賃を頼む。あいつの船だ、手は抜けない。',
...
Object.assign(DATA.QUESTS2, {
  ...
  b_void: { npcName:'虚無の囁き', type:'survive', enemy:'voidwisp', time:25,
    ...
    done:['……戻った。この庵の力を、お前にも分けよう。','……ときに。嵐の塔の鐘が、独りでに鳴るわけを知りたくはないか。','あれは終焉の潮騒に共鳴しておる。リーパーの群れが世界の縁で膨らむとき、世界はわずかに軋む。その軋みを、あの鐘は聞いておるのだ。','鐘が鳴った夜は、備えよ。……世界の終わりは、いつも音から始まる。','(この基地のパワーアップ施設が使えるようになった!)'],
    reward:{ coins:550 } },
});
...
// 各基地の背景設定に沿ったテンプレートで、未定義の基地ぶんを生成する。
{
  ...
  const FACQ = {
    ...
    b_ember:['lavaslime',6,'御神火にラヴァスライムが飛び込んで、火が濁っちゃったんだ。','御神火が澄んだよ!火の民の力、きみにも分けてあげる!',
...
DATA.MEMOIRS = [
  ...
  '「九頁目 ― 明日、果ての城に入る。戻れない気がする。だからこの手記を、見晴らしの岩に残していく」',
...
DATA.NPC_TIPS = [
  ...
  '旗の立つ見晴らしの高岩に登ると、あたりの地形が地図に刻まれる。岩の割れ目に古い手記が挟まっていることもあるそうだ。',
