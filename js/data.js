...
DATA.SIDEQUESTS = {
  ...
  b_south: [
    { id:'sq_mama', npc:'npc_girl', npcName:'宿屋の女将マーサ', type:'hunt', enemy:'mush', count:8,
      intro:['あら旅の人、うちは巡礼さん相手の宿屋なの。','裏の森のマイコニドが food…じゃなくて食料庫を荒らすのよ。8体お願い!'],
      done:['助かったわ〜。そうだ、南の海の向こうに「深緑の社」があるの。','巡礼さんたちの目的地よ。地図に描いてあげる。'],
  ...
  b_forge: [
    { id:'sq_hagane', npc:'npc_smith', npcName:'弟子ハガネ', type:'delivery', need:{ mats:{ scrap:20 } },
      ...
      done:['いい火だ…!これで神鉄が打てる。','親方からの伝言だ。「腕を磨いて、また来い」。'],
  ...
  b_moon: [
    { id:'sq_luna', npc:'npc_miko', npcName:'修道女ルナ', type:'hunt', enemy:'shade', count:10,
      ...
      done:['月光が戻りました…祈りの中で視えたのです。','南の大陸の「墓標の祭壇」に、大きな悲しみが集っている、と。'],
  ...
  b_end: [
    { id:'sq_ou', npc:'npc_sage', npcName:'城主オウ', type:'hunt', enemy:'hornedimp', count:10,
      ...
      intro:['隠者の導きで来たか。余はこの最果ての城の主。','城内にデーモンどもが巣食った。10体、討ち払え。'],
      done:['見事だ。お前はこの世界の果てまで辿り着いた、数少ない魂だ。','…だが物語はまだ終わらぬ。終焉の刻を超えた先で、また会おう。'],
...
DATA.QUESTS = {
  ...
  b_north: { npc:'npc_elder', npcName:'老兵ガルド', type:'hunt', enemy:'skeleton', count:4,
    ...
    done:['見事じゃ…!砦の狼煙を上げるぞ。','…お前さん、「死に戻り」の魂じゃな。ワシら年寄りには分かる。','命を落としても魂は広場に還り、そこで研いだ力だけが次の生へ持ち越される――そういう質の魂じゃ。','お前さんの魂の広場と、この砦が繋がった。いつでも戻ってこい。'] },
  ...
  b_end:  { npc:'npc_elder', npcName:'最果ての賢者', type:'survive', enemy:'reaper', time:25,
    ...
    done:['…見届けた。30分――それがこの世界が一つの周回を保てる限界。','刻が満ちれば、世界の縁からリーパーが押し寄せ、全てを刈り取っていく。','その先に何があるかは、この城の主だけが知っておる。','お前こそ、終焉に抗う者。全てを解き放とう。'] },
};
...
DATA.QUESTS2 = {
  ...
  b_east: { npcName:'考古学者リナ', offer:'遺跡の最深部を調査したいの。護衛、お願いできない?',
    ...
    done:['解析完了!…すごいわ、これ身体能力を強化する術式よ。あなたにも使えるはず。','いい?素材は魔物の力の結晶。それを編んで身に宿すのが「スキル」。周回の中でしか編めなくて、死ねばほどける。','でも「編み方」の記憶だけは魂に残る。各地の書庫の写本は、その編み方を研ぎ澄ませるためのものよ。','最深部の装置も動き出したわ。(この基地のパワーアップ施設が使えるようになった!)'],
...
DATA.STATIONS = {
  altar: { name:'強化の祭壇',   sprite:'st_altar', desc:'魂に刻まれた戦いの記憶を研ぎ直す(基礎能力)' },
  lab:   { name:'素材研究所',   sprite:'st_lab',   desc:'魔物の素材に宿る力を解き明かす(素材と経済)' },
  camp:  { name:'仲間の宿舎',   sprite:'st_camp',  desc:'共鳴した魂たちが暮らす場所(仲間の強化)' },
  lib:   { name:'スキル書庫',   sprite:'st_lib',   desc:'心得と秘術の写本を集めた書庫(スキル)' },
};
